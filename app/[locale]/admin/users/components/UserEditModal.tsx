...
async function getAssignableRoles(currentUserId: string, allRoles: Role[]) {
  ...
  return allRoles.filter((role) => result.results?.[role.name] === true);
}
...
export default function UserEditModal({
  ...
  const hasOriginalImage =
    originalProfileImage !== null && originalProfileImage !== "";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.error(
        error instanceof Error ? error.message : t("notifications.updateError")
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium text-amber-800">
                          {isImageRemoved
                            ? t("profileImage.status.removed")
                            : t("profileImage.status.changed")}
                        </p>
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <div className="space-y-2">
                        <Label htmlFor="confirm-password">
                          {t("password.confirmPasswordLabel")}
                        </Label>
                        <div className="relative">
                          <Input
                            id="confirm-password"
            ...
            <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all duration-200 rounded-xl">
              ...
              <CardContent>
                  ...
                  <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                    <div className="flex items-center gap-2 text-amber-800">
                      ...
                      <span className="text-sm font-medium">
                        {t("roleAssignment.securityWarningTitle")}
                      </span>
                    </div>
                ...
                {isEditingSelf && isSuperAdmin && (
                  <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <div className="flex items-center gap-2 text-blue-800">
                      <Shield className="h-4 w-4" />
                      <span className="text-sm font-medium">
                        {t("roleAssignment.superAdminWarningTitle")}
                      </span>
                    </div>
                ...
                <div className="space-y-3">
                      ...
                      return (
                        <div
                          ...
                          className={`flex items-center space-x-3 p-3 rounded-lg border ${
                            canModifyRole ? "hover:bg-muted/50" : "opacity-60"
                          }`}
                        >
                          <Checkbox
                            id={`role-${role.id}`}
                  ...
                  {roles.filter(
                    (role) => role.isActive && !availableRoles.includes(role)
                  ).length > 0 && (
                    <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="flex items-center gap-2 text-blue-800">
                        <Shield className="h-4 w-4" />
                        <span className="text-sm font-medium">
                          {t("roleAssignment.restrictedRolesTitle")}
                        </span>
                      </div>
                      <p className="text-sm text-blue-700 mt-1">
                ...
                {!selectedRole && (
                  <div className="text-center py-4 text-muted-foreground">
                    ...
                    <p className="text-sm">
                      {t("roleAssignment.noRoleSelected")}
                    </p>
                  </div>
                )}
                {selectedRole && (
                  <div className="mt-4 pt-4 border-t">
                    <Label className="text-sm font-medium">
                      {t("roleAssignment.selectedRoleLabel")}
                    </Label>
      ...
      <Dialog open={imageViewerOpen} onOpenChange={setImageViewerOpen}>
        <DialogContent className="max-w-xl w-[60vw] h-[60vh] p-0 overflow-hidden bg-slate-900 border border-gray-800">
          <div className="relative w-full h-full bg-black/95 flex items-center justify-center">
            <div className="relative max-w-full max-h-full">
              {imageToDisplay && (
                <Image
                  src={imageToDisplay}
                  alt={t("imageViewer.title", {

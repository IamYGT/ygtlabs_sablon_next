...
export default function UserActionsCell({
  ...
  const handleToggleUserStatus = async () => {
    ...
    try {
      ...
      if (response.ok) {
        toast.success(
          user.isActive ? t("userDeactivated") : t("userActivated")
        );
        onUserUpdate();
      } else {
  ...
  return (
    <>
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="end"
          className="bg-blue-50 dark:bg-slate-800 border border-gray-200 dark:border-gray-700 shadow-lg"
        >
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {t("roleManagementDesc", {
                userName: user.name || t("unnamedUser"),
              })}
            </DialogDescription>
          </DialogHeader>
            ...
            <div>
              ...
              {user.roleAssignments.length > 0 ? (
                user.roleAssignments.map((assignment) => (
                  <div
                    key={assignment.id}
                    className="flex items-center justify-between p-2 my-1 bg-gray-100 dark:bg-gray-800 rounded"
                  >
